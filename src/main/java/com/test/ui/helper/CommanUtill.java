...
public class CommanUtill extends GeneralBrowserSetting {
	...
	static {
        try {
            ...
            System.setProperty("current.date", new java.text.SimpleDateFormat("yyyyMMdd_HHmmss").format(new java.util.Date()));

            // Try to load log4j configuration from the classpath first (works when resources are
            // on the test/classpath after Maven/IDE resource copying). If not found, fall back to
            // the src/test/resources file path (useful during local dev runs).
            java.net.URL configUrl = CommanUtill.class.getClassLoader().getResource("log4j.properties");
            if (configUrl != null) {
                org.apache.log4j.PropertyConfigurator.configure(configUrl);
                System.out.println("Log4j initialized from classpath: " + configUrl);
            } else {
                // Fallback to previous behavior (file path). This may be required when running
                // from IDE working directory where resources aren't on the classpath.
                String filePath = "src/test/resources/log4j.properties";
                java.io.File f = new java.io.File(filePath);
                if (f.exists()) {
                    org.apache.log4j.PropertyConfigurator.configure(filePath);
                    System.out.println("Log4j initialized from file: " + filePath);
                } else {
                    System.err.println("log4j.properties not found on classpath or at: " + filePath);
                }
            }

            System.out.println("Log4j initialization completed. Log files will be created inside LogFileReport folder.");
        } catch (Exception e) {
	...
	// === Step-wise Logging (for Test Steps Tracking) ===
	private static int stepCounter = 1;

	public static void logStep(String message) {
	    String formattedMessage = "[STEP-" + stepCounter + "] " + message;
	...
	*/
	...
	public static void textEnter(String xmlPath, String inputText) throws IOException, InterruptedException
	{
	    ...
	    if (inputText.toString().equals("")) {
	        ...
	        try {
	            ...
	            passedScreenShotPics(driver);
	            WebElement element = driver.findElement(By.xpath(xmlPath));
	            highlightElement(element, "violet");
	            element.clear();
	            Thread.sleep(500);
	...
	public static void dropdownSelectByVisibleText(String xmlPath, String dropdronText) throws IOException, InterruptedException {
	    ...
	    if (dropdronText.toString().equals("")) {
	        System.out.println(xmlPath + ": No Dropdown value");
	        logger.log(Status.INFO, " ( " + xmlPath + " ) No Dropdown Value");
	        passedScreenShotPics(driver); // Capturing Screenshot
	    } else {
	        try {
	            System.out.println("Start Selecting ...");
	            logger.log(Status.INFO, "Start Selecting...");
	            System.out.println(xmlPath + ": Dropdown Value : (" + dropdronText + ")");
	            logger.log(Status.INFO, xmlPath + ": Dropdown Value : (" + dropdronText + ")");
	            passedScreenShotPics(driver);

	            WebElement element = driver.findElement(By.xpath(xmlPath));
	            highlightElement(element, "blue"); // highlight in blue
	...
	//update code 05-oct-2025
	
	 public static String generateRandomText(String baseName) {
	        int randomNum = (int)(Math.random() * 10000);  // 4 digit random number
	   ...
	   // 
		...
		public static void rightClickOnElement(String xpath ,String fleldName) {
	        try {
	            ...
	            System.out.println("Right click performed: " + fleldName);
	        } catch (Exception e) {
	            System.err.println("Failed to right click on element: " + fleldName);
	            e.printStackTrace();
	        }
	    ...
	    /**
	     * Click on a WebElement safely.
	     */
		    public static void clickFunction1(WebElement element) throws IOException, InterruptedException {
		    ...
		    public static WebDriver getDriver() {
		        return driver; // jo aapke CommanUtill me store hai
		    }
		    
//update Press keys
		    public static void enterKeyPress(String xpath) throws IOException {
		        WebElement element = driver.findElement(By.xpath(xpath));
...
//SwitchToAnotherWindow 23/10/2025
		 public static void switchToAnotherWindow(String xmlPath, String fieldName) throws IOException, InterruptedException {
			 
		     try {
		         System.out.println("Start Switching to Another Window from (" + fieldName + ")");
		         logger.log(Status.INFO, "Start Switching to Another Window from (" + fieldName + ")");
		         passedScreenShotPics(driver);
		         ...
		         for (String window : allWindows) {
		             if (!window.equals(parentWindow)) {
		                 ...
		                 logger.log(Status.PASS, "Switched to New Window: " + driver.getTitle());
		                 break;
		             }
		         }
		         ...
		         System.out.println("XPath-->> (" + xmlPath + " )");
		         logger.log(Status.INFO, "( " + xmlPath + " )");
		         System.out.println("Completed Switching to Another Window for (" + fieldName + ")");
		         logger.log(Status.INFO, "Completed Switching to Another Window for (" + fieldName + ")");
		         Thread.sleep(300);

		     } catch (Exception e) {
		         System.out.println("Error while Switching Window for (" + fieldName + "): " + e.getMessage());
		         logger.log(Status.FAIL, "Error while Switching Window for (" + fieldName + "): " + e.getMessage());
		         failureScreenShotPics(driver);
		         Assert.fail("Error while Switching Window for (" + fieldName + ")");
		     }
		 }
		 
	//switch back to the parent window	23/10/2025 
		 public static void switchBackToParentWindow(String xmlPath, String fieldName) throws IOException, InterruptedException {
			 
			    try {
			        System.out.println("Start Switching Back to Parent Window from (" + fieldName + ")");
			        logger.log(Status.INFO, "Start Switching Back to Parent Window from (" + fieldName + ")");
			        passedScreenShotPics(driver);
			        ...
			        logger.log(Status.PASS, "Switched Back to Parent Window: " + driver.getTitle());

			        System.out.println("XPath-->> (" + xmlPath + " )");
			        logger.log(Status.INFO, "( " + xmlPath + " )");
			        System.out.println("Completed Switching Back for (" + fieldName + ")");
			        logger.log(Status.INFO, "Completed Switching Back for (" + fieldName + ")");
			        Thread.sleep(300);

			    } catch (Exception e) {
			        System.out.println("Error while Switching Back Window for (" + fieldName + "): " + e.getMessage());
			        logger.log(Status.FAIL, "Error while Switching Back Window for (" + fieldName + "): " + e.getMessage());
			        failureScreenShotPics(driver);
